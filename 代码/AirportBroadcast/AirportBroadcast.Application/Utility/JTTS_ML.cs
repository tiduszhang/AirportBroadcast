/*-----------------------------------
 * Copyright (C) 2015 南京特捷交通技术系统有限公司
 * 版权所有。
 * 功能描述：系统语音功能类
 * 文件：Jtts.cs
 * 类名：Jtts
 * 命名空间：TTS.SelfServiceEquipment.Common
 * 
 * 创建标识：尚春城 2015/07/02
 * 
 * 修改标识：
...
namespace AirportBroadcast.Utility
{
    public class VoiceBroadcastUtil 
    {
        ...
        /// <returns></returns>
        public static int PlayText(string textStr)
        {
            VoiceBroadcastUtil.jTTS_Stop();
            return VoiceBroadcastUtil.jTTS_Play(textStr, 0);
        }

        public static int PlayTextToFile(string textStr,string fileName)
        {
            JTTS_CONFIG c = new JTTS_CONFIG();
            int iErr = jTTS_Get(out c);
            return VoiceBroadcastUtil.jTTS_PlayToFile(textStr, fileName, FORMAT_WAV, ref c, 0,0, 0);
        }
