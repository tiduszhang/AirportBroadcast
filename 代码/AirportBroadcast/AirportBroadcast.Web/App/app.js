...
var appModule = angular.module("app", [
    ...
    'ui.grid.autoResize', //解决不同的tab中使用ui-grid 消失的bug
    "oc.lazyLoad",
    "ngSanitize",
    'angularFileUpload',
    'daterangepicker',
...
/* THEME SETTINGS */
App.setAssetsPath(abp.appPath + 'metronic/ttsassets/');
...
appModule.config([
    ...
    function ($stateProvider, $urlRouterProvider, $qProvider) {
        ...
        if (abp.auth.hasPermission('Pages.Tenant.Dashboard')) {
            ...
            $stateProvider.state('tenant.dashboard.arr', {
                url: '/dashboardArr',
                views: {
                    'myhz-views': {
                        templateUrl: '~/App/tenant/views/dashboard/arr/arr.cshtml'
                    }
                }
            });

            $stateProvider.state('tenant.dashboard.dep', {
                url: '/dashboardDep',
                views: {
                    'myhz-views': {
                        templateUrl: '~/App/tenant/views/dashboard/dep/dep.cshtml'
                    }
                }
            });
