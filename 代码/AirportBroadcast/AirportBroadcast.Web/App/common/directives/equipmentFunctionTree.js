 ...
 */
appModule.directive('equipmentFunctionTree', [
      function () {
          return {
              restrict: 'E',
              template: '<div class="equipment-function-tree"></div>',
              scope: {
                  editData: '='
              },
              link: function ($scope, element, attr) {

                  var $tree = $(element).find('.equipment-function-tree');
                  ...
                  function initializeTree() {
                      ...
                      var treeData = _.map($scope.editData.equipmentFunctions, function (item) {
                          return {
                              id: item.id,
                              parent: item.parentFunctionId ? item.parentFunctionId : '#',
                              text: item.functionName,
                              state: {
                                  opened: true,
                                  selected: _.contains($scope.editData.grantedEquipmentFunctions, item.id)
                              }
                          };
                      });
                      ...
                      $tree.on("changed.jstree", function (e, data) {
                          ...
                          if (!wasInTreeChangeEvent) {
                              $scope.$apply(function () {
                                  $scope.editData.grantedEquipmentFunctions = getSelectedPermissionNames();
                              });
