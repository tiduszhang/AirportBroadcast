...
namespace AirportBroadcast.Notifications
{
    public class AppNotifier : AbpZeroTemplateDomainServiceBase, IAppNotifier
    {
        ...
        public async Task ExceptionAsync(Exception exception)
        {
            var notificationData = new LocalizableMessageNotificationData(
                new LocalizableString(
                    "ExceptionNotificationMessage",
                    AbpZeroTemplateConsts.LocalizationSourceName
                    )
                );

            notificationData["message"] = exception.Message;
            if (exception.InnerException != null)
            {
                notificationData["innermessage"] = exception.InnerException.Message;
            }
            else
            {
                notificationData["innermessage"] ="";
            }
            await _notificationPublisher.PublishAsync(AppNotificationNames.Exception, notificationData);
        }
