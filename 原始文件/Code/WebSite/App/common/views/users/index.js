(function () {
    ...
    appModule.controller('common.views.users.index', [
        ...
        function ($scope, $uibModal, $stateParams, uiGridConstants, userService) {
                ...
                columnDefs: [
                    {
                        ...
                        cellTemplate:
                            ...
                            '      <li><a ng-if="grid.appScope.permissions.impersonation && row.entity.id != grid.appScope.currentUserId" ng-click="grid.appScope.impersonate(row.entity)">登录此用户</a></li>' +
                            ...
                            '      <li><a ng-click="grid.appScope.unlockUser(row.entity)">解锁</a></li>' +
            ...
            vm.editCarryStation = function (user) {
                if (user.isCanSetAllCarryStation) {
                    abp.message.warn("已可管理所有客运站，无需分配！", "");
                    return;
                }

                var modalInstance = $uibModal.open({
                    templateUrl: '~/App/common/views/users/editUserCarrystationModal.cshtml',
                    controller: 'common.views.users.editUserCarrystationModal as vm',
                    backdrop: 'static',
                    resolve: {
                        userId: function () {
                            return user.id;
